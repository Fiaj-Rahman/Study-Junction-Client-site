...
const Login = () => {
  ...
  const handleSocialLogin = async (socialProvider) => {
    try {
      ...
      if (result.user) 
        {
          const {data} = await axios.post(`https://assignment11-server-site-peach.vercel.app/jwt`,{
  ...
  const onSubmit = (data) => {
    ...
    signInUser(email, password)
      .then((result) => {
        if (result.user) {
          return axios.post(`https://assignment11-server-site-peach.vercel.app/jwt`, {
            email: result?.user?.email,
          }, { withCredentials: true });
        }
      })
      ...
      .catch((error) => {
        setEmptyError(error.message.split("/")[1]);
      });
  };
